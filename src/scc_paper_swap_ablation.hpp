#ifndef SCC_PAPER_SWAP_ABLATION_H
#define SCC_PAPER_SWAP_ABLATION_H
...
struct AblationMetrics {
  uint64_t batch_edge_checks = 0;
  uint64_t real_link_checks = 0;
  uint64_t fake_link_checks = 0;
  uint64_t partition_edge_checks = 0;
  uint64_t pivot_validation_edge_checks = 0;
  uint64_t scope_reused_scans = 0;
  uint64_t scope_updates = 0;
  uint64_t partitioned_lists = 0;
  uint64_t swapped_entries = 0;
  uint64_t pivot_candidates_examined = 0;
  uint64_t pivots_selected = 0;
  uint64_t resolved_candidates_skipped = 0;
  uint64_t singleton_candidates_finalized = 0;
  uint64_t refill_candidates_examined = 0;
  uint64_t initial_forward_rounds = 0;
  uint64_t initial_backward_rounds = 0;
  uint64_t batch_forward_rounds = 0;
  uint64_t batch_backward_rounds = 0;
  uint64_t executed_batches = 0;
  uint64_t table_resizes = 0;
  uint64_t baseline_invalid_candidates = 0;
};

struct alignas(64) AblationMetricShard {
  uint64_t batch_edge_checks = 0;
  uint64_t real_link_checks = 0;
  uint64_t fake_link_checks = 0;
  uint64_t partition_edge_checks = 0;
...
class SCC {
 private:
  ...
  sequence<AblationMetricShard> metric_shards;
  ...
  bool enable_fake_links = true;
  bool enable_bounded_pivot = true;
  bool emulate_gbbs_vgc = false;
  AblationMetrics run_metrics;
 ...
 public:
  ...
  void set_ablation(bool fake_links, bool bounded_pivot,
                    bool gbbs_vgc_compatibility = false) {
    enable_fake_links = fake_links;
    enable_bounded_pivot = bounded_pivot;
    emulate_gbbs_vgc = gbbs_vgc_compatibility;
  }
  AblationMetrics metrics() const;
  SCC() = delete;
  SCC(Graph& G) : graph(G), bag(G.n, _min_bag_size_) {
    ...
    metric_shards = sequence<AblationMetricShard>(
...
void SCC::reset_run_metrics() {
  run_metrics = AblationMetrics{};
  parallel_for(0, metric_shards.size(),
               [&](size_t i) { metric_shards[i] = AblationMetricShard{}; });
}

AblationMetrics SCC::metrics() const {
  AblationMetrics result = run_metrics;
  for (size_t i = 0; i < metric_shards.size(); ++i) {
    const auto& shard = metric_shards[i];
    result.batch_edge_checks += shard.batch_edge_checks;
    result.real_link_checks += shard.real_link_checks;
    result.fake_link_checks += shard.fake_link_checks;
    result.partition_edge_checks += shard.partition_edge_checks;
    result.pivot_validation_edge_checks +=
        shard.pivot_validation_edge_checks;
    result.scope_reused_scans += shard.scope_reused_scans;
    result.scope_updates += shard.scope_updates;
    result.partitioned_lists += shard.partitioned_lists;
    result.swapped_entries += shard.swapped_entries;
    result.pivot_candidates_examined += shard.pivot_candidates_examined;
    result.pivots_selected += shard.pivots_selected;
    result.resolved_candidates_skipped +=
        shard.resolved_candidates_skipped;
    result.singleton_candidates_finalized +=
        shard.singleton_candidates_finalized;
    result.refill_candidates_examined += shard.refill_candidates_examined;
  }
  return result;
}
...
void SCC::identify_fake_links(NodeId v, bool forward, EdgeId scan_begin,
  ...
  ++metric_shards[parlay::worker_id()].scope_updates;
}
...
int SCC::multi_search(sequence<size_t>& labels,
  ...
  while (sub_n_front > 0) {
    ...
    parallel_for(
        ...
        [&](size_t i) {
          ...
          if (local && (degree < queue_size) &&
            ...
            while (head < tail && edge_map_cnt < queue_size) {
              ...
              auto& metric = metric_shards[parlay::worker_id()];
              if (scope_is_known) ++metric.scope_reused_scans;
              for (EdgeId j = begin; j < end; j++) {
                NodeId v = E[j];
                const size_t target_label = labels[v];
                ++metric.batch_edge_checks;
                if (!(target_label & TOP_BIT) &&
                    target_label == source_label) {
                  ++metric.real_link_checks;
                  ...
                  ++metric.fake_link_checks;
                }
              }
              if (enable_fake_links && (real_count != deg || deg == 0)) {
            ...
            if (node_scope_known) {
              ++metric_shards[parlay::worker_id()].scope_reused_scans;
            }
            parallel_for(
                0, degree,
                [&](size_t j) {
                  NodeId ngb_node = E[node_begin + j];
                  const size_t target_label = labels[ngb_node];
                  auto& metric = metric_shards[parlay::worker_id()];
                  ++metric.batch_edge_checks;
                  if (!(target_label & TOP_BIT) &&
        ...
        1);
    if (enable_fake_links && local && sub_round == 1) {
      ...
      parallel_for(
          ...
          [&](size_t i) {
            ...
            EdgeId j = begin;
            auto& metric = metric_shards[parlay::worker_id()];
            ++metric.partitioned_lists;
            while (j < end) {
              const NodeId v = E[j];
              const size_t target_label = labels[v];
              ++metric.partition_edge_checks;
              ++metric.batch_edge_checks;
              if ((target_label & TOP_BIT) ||
                  target_label != source_label) {
                ++metric.fake_link_checks;
...
void SCC::scc(sequence<size_t>& labels, double beta, bool local_reach,
  ...
  run_metrics.initial_forward_rounds = fowd_depth;
  // backward search
  REACH_P.swap_graph();
  int bawd_depth = REACH_P.reach(source, dist_2, local_reach);
  run_metrics.initial_backward_rounds = bawd_depth;
  #ifdef ROUND
  ...
  while (cur_offset < n_remaining) {
    ...
    if (enable_bounded_pivot) {
      const NodeId initial_limit = cur_offset + target_size;
      const NodeId refill_budget = std::min<NodeId>(
          4096, std::max<NodeId>(1, target_size / 64));
      const NodeId scan_limit =
          std::min(n_remaining, initial_limit + refill_budget);

      while (n_front < target_size && cur_offset < scan_limit) {
        const bool is_refill = cur_offset >= initial_limit;
        ...
        parallel_for(0, candidates.size(), [&](size_t i) {
          ...
          auto& metric = metric_shards[parlay::worker_id()];
          ++metric.pivot_candidates_examined;
          if (is_refill) ++metric.refill_candidates_examined;
          if (__atomic_load_n(&labels[v], __ATOMIC_RELAXED) & TOP_BIT) {
            pivot_valid[v] = 0;
            ++metric.resolved_candidates_skipped;
          } else if (enable_fake_links &&
                     is_potential_singleton(v, labels)) {
            ...
            __atomic_store_n(&labels[v], singleton_label | TOP_BIT,
                             __ATOMIC_RELAXED);
            pivot_valid[v] = 0;
            ++metric.singleton_candidates_finalized;
          } else {
            pivot_valid[v] = 1;
          }
        });
        ...
        parallel_for(0, selected.size(), [&](size_t i) {
          front[n_front + i] = selected[i];
          ++metric_shards[parlay::worker_id()].pivots_selected;
        });
        n_front += selected.size();
        cur_offset = end;
      }
    } else {
      const NodeId end = std::min(cur_offset + target_size, n_remaining);
      auto candidates = centers.cut(cur_offset, end);
      if (emulate_gbbs_vgc) {
        auto pred = delayed_seq<bool>(candidates.size(), [&](size_t i) {
          return !(__atomic_load_n(&labels[candidates[i]], __ATOMIC_RELAXED) &
                   TOP_BIT);
        });
        n_front = pack_into_uninitialized(candidates, pred, make_slice(front));
        run_metrics.pivot_candidates_examined += candidates.size();
        run_metrics.pivots_selected += n_front;
        run_metrics.resolved_candidates_skipped +=
            candidates.size() - n_front;
      } else {
        parallel_for(0, candidates.size(), [&](size_t i) {
          const NodeId v = candidates[i];
          auto& metric = metric_shards[parlay::worker_id()];
      ...
      // The published GBBS-VGC executable performs this candidate-validity
      // scan after fixed-segment pivot selection. Its result is diagnostic,
      // but the work is inside the baseline's measured SCC call and must be
      // retained when reproducing that baseline's running time.
      if (emulate_gbbs_vgc) {
        sequence<bool> candidate_valid(n_front);
        parallel_for(0, n_front, [&](size_t i) {
          const NodeId v = front[i];
          bool has_out = false;
          bool has_in = false;
          auto& metric = metric_shards[parlay::worker_id()];
          for (EdgeId j = graph.offset[v]; j < graph.offset[v + 1]; ++j) {
            ++metric.batch_edge_checks;
            ++metric.pivot_validation_edge_checks;
            if (!(labels[graph.E[j]] & TOP_BIT)) {
              has_out = true;
              break;
            }
          }
          for (EdgeId j = graph.in_offset[v]; j < graph.in_offset[v + 1];
               ++j) {
            ++metric.batch_edge_checks;
            ++metric.pivot_validation_edge_checks;
            if (!(labels[graph.in_E[j]] & TOP_BIT)) {
              has_in = true;
              break;
            }
          }
          candidate_valid[i] = has_out && has_in;
        });
        auto invalid = parlay::filter(
            candidate_valid, [](bool valid) { return !valid; });
        run_metrics.baseline_invalid_candidates +=
            invalid.size() + (candidates.size() - n_front);
      }
      cur_offset = end;
    }
    ...
    size_t out_table_m = (emulate_gbbs_vgc || conservative_tables)
                             ? 2 * out_table_base
                             : (5 * out_table_base) / 4;
    table_forw =
    ...
    run_metrics.batch_forward_rounds += out_depth;
    t_search.stop();
    t_search.start();
    ...
    size_t in_table_m = (emulate_gbbs_vgc || conservative_tables)
                            ? 2 * in_table_base
                            : (5 * in_table_base) / 4;
    table_back =
    ...
    run_metrics.batch_backward_rounds += in_depth;
    t_search.stop();
