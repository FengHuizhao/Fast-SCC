#include "scc_paper_swap_fig10.hpp"
...
int main(int argc, char** argv) {
  ...
  // Dense large graphs benefit from a larger local-search queue.  Keep the
  // smaller queue for sparse graphs, where it avoids over-expanding a batch;
  // an explicit -tau always overrides this heuristic.
  int default_tau = (graph.m >= 200000000) ? 2048 : 512;
  ...
  timer t;
  SCC SCC_P(graph);
  cout << "FIG10_SCHEMA variant,run,batch,forward_depth,backward_depth" << endl;
  cout << "FIG10_NOTE initial single-pivot search is reported separately and excluded from Figure 10 batch averages" << endl;
  cout << "FIG10_WARMUP_BEGIN variant=Fast-SCC" << endl;
  SCC_P.scc(label, beta, local_reach, local_scc);
  cout << "FIG10_WARMUP_END variant=Fast-SCC" << endl;

  SCC_P.timer_reset();
  double scc_cost;
  for (int i = 0; i < repeat; i++) {
    cout << "FIG10_RUN_BEGIN variant=Fast-SCC run=" << (i + 1) << endl;
    t.start();
    SCC_P.scc(label, beta, local_reach, local_scc);
    scc_cost = t.stop();
    cout << "scc cost: " << scc_cost << endl;
    cout << "FIG10_RUN_END variant=Fast-SCC run=" << (i + 1)
         << " runtime_seconds=" << scc_cost << endl;
  }
